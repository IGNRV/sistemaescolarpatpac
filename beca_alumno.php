...
$mediosDePago = [];
if (!empty($rutTutor)) {
    $stmtMediosDePago = $conn->prepare("SELECT ID_BECA, RUT_ALUMNO, DESCUENTO, ESTADO_BECA, PERIODO_ESCOLAR FROM BECAS WHERE RUT_ALUMNO = ?");
...
if (isset($_POST['INGRESAR_DATOS'])) {
    $medioPago = $_POST['descuento'];
    $bancoEmisor = $_POST['periodoescolar'];
    ...
    $stmt = $conn->prepare("SELECT (ID_ALUMNO) AS ultimo_numero FROM MEDIOS_DE_PAGO");
    $stmt->execute();
    $resultado = $stmt->get_result();
    ...
    $stmt = $conn->prepare("INSERT INTO BECAS (MEDIO_PAGO, BANCO_EMISOR, TIPO_MEDIOPAGO, RUT_PAGADOR, NRO_MEDIOPAGO, FECHA_SUSCRIPCION, ESTADO_MP, FECHA_VENCIMIENTO_MP, FECHA_INGRESO, FECHA_ACTIVACION, PERIODO_ESCOLAR) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
if (isset($_POST['ACTUALIZAR_DATOS'])) { // Asegúrate de que este nombre de input coincide con el atributo "name" del botón de actualización
    ...
    if ($stmtActualizar->affected_rows > 0) {
        $mensaje = "Datos de la beca actulizados correctamente.";
    } else {
        $mensaje = "Error al actualizar los datos de la beca.";
    }
