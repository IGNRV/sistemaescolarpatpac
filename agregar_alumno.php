...
$periodosEscolares = [];
$stmtPeriodos = $conn->prepare("SELECT ID, PERIODO FROM PERIODO_ESCOLAR ORDER BY PERIODO DESC");
...
if (isset($_POST['agregarAlumno'])) {
    ...
    if (isset($_FILES['fotoAlumno']) && $_FILES['fotoAlumno']['error'] == 0) {
        $directorioDestino = "/home/pat-pac/domains/antilen.pat-pac.cl/public_html/sistemaescolar/fperfil/";
        $nombreArchivo = basename($_FILES['fotoAlumno']['name']);
        $rutaCompleta = $directorioDestino . $nombreArchivo;

        if (move_uploaded_file($_FILES['fotoAlumno']['tmp_name'], $rutaCompleta)) {
            // El archivo se subió correctamente
            $fotoAlumno = $nombreArchivo;
        } else {
            // Error al subir el archivo
            $mensaje = "Error al subir la foto del alumno.";
            $fotoAlumno = "";
        }
    } else {
        // No se subió ningún archivo o hubo un error
        $fotoAlumno = ""; // O manejar el error como prefieras
    }
    ...
    // Añade la fecha especial si la fecha de ingreso es en enero o febrero del año seleccionado
    $fechaEspecial = ($anioPeriodoEscolar - 1) . "-12-22";
    if (($mesFechaIngreso == '01' || $mesFechaIngreso == '02') && $anioFechaIngreso == $anioPeriodoEscolar) {
        array_unshift($fechasVencimiento, $fechaEspecial); // Añade la fecha al inicio del array
    } else if ($mesFechaIngreso == '12' && $anioFechaIngreso == $anioPeriodoEscolar - 1) {
